...
class PotentialFieldPlanner:
    ...
    def attractive_force(target, current):
        ...
        # correction so that norm < 1
        if (norm > 1):
            att_f = (distance / norm)
        else:
            att_f =  (k_att) * (distance)
    ...
    def repulsive_force(obstacle, current, unitvec=np.zeros((3,1))):
        ...
        rho_grad = np.linalg.norm(dist)

        if (rho_grad > 0) & (rho_grad < rho_0):
            rep_f = -(eta) * (1.0/rho_grad - 1.0/rho_0) * (1/(rho_grad ** 2)) * (unitvec)
    ...
    def compute_forces(target, obstacle, current):
        ...
        for i in range(7):
            target_1 = target[:, i].reshape(3,1)
            current_1 = current[:, i].reshape(3,1)
            F_att = PotentialFieldPlanner.attractive_force(target_1, current_1).flatten()
            F_net = F_att
            for j in range (len(obstacle)):
                F_rep = PotentialFieldPlanner.repulsive_force(obstacle[j,:], current_1).flatten()
                F_net = F_net + F_rep
            joint_forces[:, i] = F_net
    ...
    def compute_torques(joint_forces, q):
        ...
        num_joints = 7
        joint_torques = np.zeros((1, num_joints))


        # Get the Jacobian for the initial configuration
        J = calcJacobian(q)
        J_subset=np.zeros((3,7))

        # Iterate over the joints to calculate torques
        for joint_idx in range(num_joints):

            # Extract the subset of the Jacobian for the first 'joint_idx + 1' joints
            J_subset [:,:joint_idx+1] = J[:3, :joint_idx + 1]

            # Calculate the torque contribution for the current joint
            torque_contribution = np.dot(J_subset.T, joint_forces[:, joint_idx])

            # Accumulate the torque contribution
            joint_torques += torque_contribution.reshape(1, -1)
    ...
    def compute_gradient(self, q, target, map_struct):
        ...
        dq = np.zeros((1, 7))
        alpha =  self.min_step_size
        obstacle = map_struct.obstacles
        q=q.flatten()

        J_current, _ = PotentialFieldPlanner.fk.forward(q)
        J_target, _ = PotentialFieldPlanner.fk.forward(target)

        # for i in range(len(obstacle)):
        force = PotentialFieldPlanner.compute_forces(J_target.T, obstacle, J_current.T)
    ...
    def plan(self, map_struct, start, goal):
        ...
        while True:
            ...
            q_grad = PotentialFieldPlanner.compute_gradient(self, q, goal, map_struct)

            # Termination Conditions
            if PotentialFieldPlanner.q_distance(goal, q) < self.tol or iteration > self.max_steps:
                break
