...
class JacobianDemo():
    ...
    def eight(t,fx=0.3,fy=0.6,rx=.1,ry=.1):
        ...
        ang_v = (np.pi/3) * f * cos(f*t)
        ang_vdes =  np.array([0, 0.0, 0.0])

        return Rdes, ang_vdes, xdes, vdes

    def ellipse(t,f=0.3,ry=.15,rz=.10):
        """
        ...
        ang =  (np.pi/8) * sin(f*t)
        r= np.array([0.0, 0.0, ang ])
        Rdes = rotvec_to_matrix(r)

        ang_v = (np.pi/4) * f * cos(f*t)
        ang_vdes =  np.array([0, 0.0, ang_v])
        ## END STUDENT CODE
