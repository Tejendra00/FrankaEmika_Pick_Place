...
class PotentialFieldPlanner:
    ...
    def repulsive_force(obstacle, current, unitvec=np.zeros((3,1))):
        ...
        if (rho_grad > 0) & (rho_grad < rho_0):
            rep_f = -(eta) * (1.0/rho_grad - 1.0/rho_0) * (1/(rho_grad ** 2)) * (unitvec.T)
    ...
    def compute_forces(target, obstacle, current):
        ...
        for i in range(7):
            ...
            for j in range (len(obstacle)):
                F_rep = PotentialFieldPlanner.repulsive_force(obstacle, current_1).flatten()
                F_net = F_net + F_rep
    ...
    def compute_torques(joint_forces, q):
        ...
        for i in range(0, 7):
            ...
            currentJv = np.zeros((3,7))
            for j in range(0, i):
                currentJv[:,j] = Jv[:,j]

            joint_torques += currentJv.T @ joint_forces[:,i]
            # joint_torques = joint_torques.flatten()
        ## END STUDENT CODE
    ...
    def compute_gradient(self, q, target, map_struct):
        ...
        for i in range(obstacle.shape[0]):
            force = PotentialFieldPlanner.compute_forces(J_target.T, obstacle[i], J_current.T)
            torque = PotentialFieldPlanner.compute_torques(force, q)
            torque_norm = (torque)/np.linalg.norm(torque)
            dq = (alpha)*(torque_norm)
            dq = dq.flatten()
    ...
    def plan(self, map_struct, start, goal):
        ...
        while True:
            ...
            if (PotentialFieldPlanner.q_distance(goal, q) > self.tol) and (iteration < self.max_steps):
                q = q + q_grad
                q_path = np.vstack((q, q_path))
                iteration += 1
